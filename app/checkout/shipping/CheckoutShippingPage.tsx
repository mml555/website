...
import { calculateTax, isValidUSAddress, US_STATES, TaxCalculation } from '@/lib/tax-calculator'
...
import { Address, BillingAddress } from '@/types/address'
...
export default function ShippingPage() {
  ...
  // TODO: Improve address management (default address selection, validation, editing, deleting)
  // TODO: Enhance shipping calculation (weight-based, international, error handling)

  // Update tax calculation when shipping state changes
  ...
  }, [cartTotal, shippingAddress.country, shippingAddress.postalCode, shippingAddress.state])
  ...
  const handleAddressChange = (
    ...
    // Convert USA to US for country field
    const processedValue = field === 'country' && value === 'USA' ? 'US' : value;
    
    if (type === 'shipping') {
      setShippingAddress(prev => ({ ...prev, [field]: processedValue }))
      if (sameAsShipping) {
        setBillingAddress(prev => ({ ...prev, [field]: field === 'email' ? processedValue : processedValue }))
      }
    } else {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // Convert any USA to US before validation
    const shippingAddressToValidate = {
      ...shippingAddress,
    ...
    if (!isValidUSAddress(shippingAddressToValidate)) {
      setError('Please enter a valid US shipping address')
      return
    }

    if (!sameAsShipping && !isValidUSAddress(billingAddressToValidate)) {
    ...
    // Prepare billing address for backend
    const billingAddressToSend = sameAsShipping ? {
    ...
    sessionStorage.setItem('billingAddress', JSON.stringify(billingAddressToSend))
    ...
    const totalWithShipping = taxCalculation 
      ? taxCalculation.total + (selectedShipping?.rate ?? 0) 
      : cartTotal + (selectedShipping?.rate ?? 0)
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <div className="flex justify-between font-medium pt-2 border-t">
                ...
                <span>${(taxCalculation 
                  ? taxCalculation.total + (selectedShipping?.rate ?? 0) 
                  : cartTotal + (selectedShipping?.rate ?? 0)).toFixed(2)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
