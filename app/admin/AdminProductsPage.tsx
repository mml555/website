...
export default function AdminProductsPage() {
  ...
  function fetchProducts(pageNum: number, limitNum: number, searchVal: string, statusVal: string) {
    setLoading(true);
    setError(null);
  ...
  // Robust calculation for pagination display
  const hasProducts = products.length > 0;
  const displayTotal = total > 0 ? total : (hasProducts ? products.length : 0);
  const from = hasProducts ? (page - 1) * limit + 1 : 0;
  const to = hasProducts ? from + products.length - 1 : 0;
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="py-8">
          {[...Array(limit)].map((_, i) => (
            <div key={i} className="animate-pulse h-10 bg-gray-100 rounded mb-2" />
          ))}
        </div>
      ) : error ? (
        <div className="text-red-600 bg-red-50 border border-red-200 rounded p-4 flex items-center gap-2">
          <svg className="w-5 h-5 text-red-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 113 12a9 9 0 0118 0z" /></svg>
          <span>{error}</span>
        </div>
      ) : products.length === 0 ? (
        ...
        {!loading && hasProducts && (
          <div className="text-sm text-gray-600 mb-2">
            Showing {from} to {to} of {displayTotal} results
          </div>
        )}
...
function PaginationControls({ page, totalPages, canPrev, canNext, setPage, limit, setLimit, jumpPage, setJumpPage, handleJumpPage }: PaginationControlsProps) {
